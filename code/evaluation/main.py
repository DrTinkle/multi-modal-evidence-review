...
def write_report(
    ...
    lines = [
        ...
        "| Local VLM evidence engine | Sends each claim and its local images to `qwen/qwen3-vl-4b`, normalizes the JSON response, merges user-history risk, caches image-hash keyed responses, logs model I/O, and validates every row before CSV writing. | See metrics below. |",
        "",
        "## Sample Metrics",
        "",
    ...
    lines.extend(
        [
            ...
            f"- Approximate model calls for test processing: {test_calls} VLM calls before cache hits.",
            ...
            "- Cost assumption: local Qwen run costs $0.00 in API fees. A hosted VLM replacement priced at $5.00/M input tokens and $15.00/M output tokens would cost roughly $0.36 for this full dataset, excluding image-token pricing differences.",
            f"- Runtime for local evaluation: {runtime_seconds:.2f} seconds on this machine.",
            "- TPM/RPM considerations: the local server is called one claim row at a time. For a hosted model, cap concurrency to the provider RPM limit, cache responses by claim and image hash, and retry only transient API failures with exponential backoff.",
            "",
            "## Final Strategy",
            "",
            "The final output uses the local VLM evidence engine in `code/main.py`. It keeps the images as the primary evidence source, merges user-history risk without letting history override clear visual evidence, ignores instruction-like text inside claims/images, logs raw model input/output per claim, and emits only values that pass the strict validator.",
        ]
    )
